...
- 생성자 기반 의존관계 주입을 선택해야 하는 이유?
    ...
    * 테스트를 쉽게 해줍니다.
    * 불변성을 확보합니다.
    * 스프링 공식문서에도 스프링 팀에서 조차 생성자 주입을 응호한다고 하고있습니다.
 
- AOP
    * 1.	로그 수행: AOP의 대표적인 사용 사례 중 하나, 메서드가 실행될 때, 메서드 호출 전후로 로그를 기록하는 데 많이 사용
      2.	예외 처리: AOP는 예외 발생 시 처리 로직을 분리하여, 예외를 처리하는 코드를 재사용할 수 있게
      3.	트랜잭션 관리: AOP는 트랜잭션 관리에도 자주 사용 - 트랜잭션을 시작하고, 메서드가 성공하면 커밋하고, 실패하면 롤백
...
-  @PathVariable : URL 경로에서 변수를 추출할 때 사용, /users/123으로 요청하면 id 변수에 123이 바인딩
-  @RequestParam : 쿼리 파라미터에서 값을 추출할 때 사용, URL에 ?key=value 형태로 전달되는 쿼리 파라미터에서 값을 추출할 때 사용
